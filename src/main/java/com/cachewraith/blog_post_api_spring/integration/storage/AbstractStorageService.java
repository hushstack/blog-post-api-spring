 ...
 *
 * <p>Template Method. The sequence — reject empty, cap the size, check the declared content type
 * against the allowlist, generate the stored name, prove the bytes really decode as an image — is
...
public abstract class AbstractStorageService implements StorageService {
    ...
    private String requireAllowedContentType(String contentType) {
        ...
        // A declared type may carry parameters (image/jpeg; charset=…); match on the type alone.
        String bare = contentType.split(";")[0].strip().toLowerCase(Locale.ROOT);
        if (!properties.getAllowedContentTypes().contains(bare)) {
            throw new BusinessException(ErrorCode.INVALID_IMAGE, "Unsupported content type");
        }
        return bare;
    }
